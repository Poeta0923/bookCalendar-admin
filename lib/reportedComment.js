...
module.exports = {
    ...
    view : (req, res)=>{
        ...
        if(login===true){
            ...
            db.query(`SELECT c.commentId, c.postId, p.title, m.nickName, c.contents, c.date, c.reportCount FROM comment c JOIN post p ON c.postId = p.postId JOIN member m ON c.memberId = m.memberId WHERE c.reportCount >= 1`, (error, results)=>{
