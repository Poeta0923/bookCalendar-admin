        ...
        if(login===true){
            const monthlyQuery = `
                ...
                FROM
                    member
                WHERE
                    registerDate >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
                GROUP BY
                    month
                ORDER BY
                    month;
            ...
            const yearlyQuery = `
                ...
                FROM
                    member
                WHERE
                    registerDate >= DATE_SUB(CURDATE(), INTERVAL 6 YEAR)
                GROUP BY
                    year
                ORDER BY
                    year;
            ...
            const monthlyGrowthQuery = `
                WITH MonthlyCounts AS (
                    ...
                    FROM
                        member
                    WHERE
                        registerDate >= DATE_SUB(CURDATE(), INTERVAL 7 MONTH)
                    GROUP BY
                        month
                    ORDER BY
                        month
            ...
            const yearlyGrowthQuery = `
                WITH YearlyCounts AS (
                    ...
                    FROM
                        member
                    WHERE
                        registerDate >= DATE_SUB(CURDATE(), INTERVAL 7 YEAR)
                    GROUP BY
                        year
                    ORDER BY
                        year
