...
module.exports = {
    ...
    search : (req, res)=>{
        ...
        if(login===true){
            ...
            db.query('SELECT p.postId, p.title, m.nickName, p.date, p.contents, p.reportCount FROM post p JOIN member m ON p.memberId = m.memberId WHERE title LIKE ? OR nickName LIKE ? OR date LIKE ? OR contents LIKE ? OR reportCount LIKE ? ORDER BY postId DESC',
