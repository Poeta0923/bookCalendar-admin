...
module.exports = {
    ...
    view : (req, res)=>{
        ...
        if(login===true){
            ...
            db.query(`SELECT p.postId, p.title, m.nickName, p.date, p.contents, p.reportCount FROM post p JOIN member m ON p.memberId = m.memberId WHERE p.reportCount>=1`, (error, results)=>{
    ...
    //'/reportedPost/:currentPage/:postId' 요청 응답
    detail : (req, res)=>{
            ...
            db.query(`SELECT p.postId, p.title, m.nickName, p.date, p.contents, p.reportCount FROM post p JOIN member m ON p.memberId = m.memberId WHERE p.reportCount >=1`, (error, results)=>{
                ...
                db.query(`SELECT p.postId, p.title, m.nickName, p.date, p.contents, p.reportCount FROM post p JOIN member m ON p.memberId = m.memberId WHERE p.postId = ?`,
                    [postId], (error, postData)=>{
                        ...
                        db.query(`SELECT m.nickName, c.commentId, c.contents, c.date, c.reportCount FROM comment c JOIN post p ON c.postId = p.postId JOIN member m ON c.memberId = m.memberId WHERE c.postId = ?`,
    ...
    //'/reportedPost/search/:currentPage' 요청 응답
    search : (req, res)=>{
        ...
        if(login===true){
            ...
            db.query('SELECT p.postId, p.title, m.nickName, p.date, p.contents, p.reportCount FROM post p JOIN member m ON p.memberId = m.memberId WHERE (title LIKE ? OR nickName LIKE ? OR date LIKE ? OR contents LIKE ? OR reportCount LIKE ?) AND reportCount >=1',
    ...
    //'reportedPost/search/:currentPage/:postId' 요청 응답
    searchDetail : (req, res)=>{
        ...
        if(login===true){
            ...
            //Server DB에 post 테이블 쿼리 요청
            db.query(`SELECT p.postId, p.title, m.nickName, p.date, p.contents, p.reportCount FROM post p JOIN member m ON p.memberId = m.memberId WHERE (title LIKE ? OR nickName LIKE ? OR date LIKE ? OR contents LIKE ? OR reportCount LIKE ?) AND p.reportCount>=1`,
                [sntzedSearchTerm, sntzedSearchTerm, sntzedSearchTerm, sntzedSearchTerm, sntzedSearchTerm], (error, results)=>{
                ...
                [postId], (error, result)=>{
                    ...
                    db.query(`DELETE FROM post WHERE postId = ?`,
                        [postId], (error, results)=>{
                            ...
                            res.redirect(`/reportedPost/${currentPage}`);
                            res.end();
                        }
                    )
                }
                ...
                [postId], (error, result)=>{
                    ...
                    db.query(`DELETE FROM post WHERE postId = ?`,
                        [postId], (error, results)=>{
                            ...
                            res.send(`
                                <form id="redirectForm" action="/reportedPost/search/${currentPage}" method="post">
        ...
        if(login===true){
            ...
            db.query(`DELETE FROM comment WHERE commentId = ?`,
                [commentId], (error, result)=>{
                    ...
                    res.redirect(`/reportedPost/${currentPage}/${postId}`);
                    res.end();
                }
            )
        }
        ...
        if(login===true){
            ...
            db.query(`DELETE FROM comment WHERE commentId = ?`,
                [commentId], (error, result)=>{
                    ...
                    // 클라이언트에게 POST 요청을 보내는 JavaScript 코드 응답
                    res.send(`
                        <form id="redirectForm" action="/reportedPost/search/${currentPage}/${postId}" method="post">
                            <input type="hidden" name="searchTerm" value="${searchTerm}">
                        </form>
                        <script>
                            document.getElementById('redirectForm').submit();
